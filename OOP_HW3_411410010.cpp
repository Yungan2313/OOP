...
class IoT_ctrl_payload : public payload {
        ...
        // bool parent;//我是不是你的parent
        bool parent_check = 0;//0 一般ctrl 1更動parent用
        unsigned int parent_recieve[2];//ID對的parent才會更動 0.增加 1.刪除
    ...
    public:
        ...
        // GET(getParent,bool,parent);
        // SET(setParent,bool,parent,_parent);
        GET(getParent_check,bool,parent_check);
        SET(setParent_check,bool,parent_check,_parent);
        unsigned int getParent_recieve(int active){return parent_recieve[active];};
        void setParent_recieve(unsigned int node,int active){parent_recieve[active] = node;};
        // GET(getTime,unsigned int,curtime);
        // SET(setTime,unsigned int,curtime,_time);
        GET(getID,unsigned int,packet_ID);
        SET(setID,unsigned int,packet_ID,_ID);
...
class IoT_device: public node {
        ...
        map<unsigned int,bool> routing_table;//(adj node,是否被拜訪過 0未被拜訪過)
        unsigned int parent;
        map<unsigned int,bool> child;//(id,小孩是否傳送封包了_data_packet)
        unsigned int level = 0;
        bool send_check = false;//檢查是否已傳送過data_packet
        // map<unsigned int,unsigned int> modify_time;//(node,modify time)
        int sequence = 0;
        map<unsigned int,unsigned int> packet_sequence;//(node,sequence)

        //hw3
        map<unsigned int, unsigned int> rev_hop;//(destination,hop_deviceID)
    protected:
    ...
    public:
        ...
        void table(unsigned int adj){routing_table.insert(std::pair<unsigned int,bool>(adj,0));};
        void view(unsigned int node){routing_table[node] = 1;};
        void data_packet_collect(unsigned int node){child[node] = 1;};
        void setchild(unsigned int node){child.insert(std::pair<unsigned int,bool>(node,0));};
...
class IoT_sink: public node {
        ...
        unsigned int parent;
        unsigned int level = 0;
        map<unsigned int,unsigned int> device_parent;
        map<unsigned int,unsigned int> next_hop;
    protected:
    ...
    public:
        ...
        // unsigned int get_one_hop_neighbor_num () { return one_hop_neighbors.size(); }
        GET(getParent,unsigned int,parent);
        unsigned int get_device_parent(int node){
            return device_parent[node];
        }
...
void IoT_device::recv_handler (packet *p){
    ...
    if (p->type() == "IoT_ctrl_packet"){ // the device receives a packet from the sink
        ...
        unsigned int packet_time = l3->getID();
        unsigned int packet_ID = p3->getHeader()->getPreID();

        if(l3->getParent_check() == 1){
            if(l3->getParent_recieve(0) == getNodeID()){//小孩傳的
                // cout << "parent " << getNodeID() << " get child " << p3->getHeader()->getPreID() << " packet time: " << packet_time << endl;
                // cout << "ok0" << endl;
                map<unsigned int,unsigned int>::iterator it;
                int flag = 0;
                for(it = packet_sequence.begin();it!=packet_sequence.end();it++){
                    if(it->first == packet_ID){
                        // cout << "ok1" << endl;
                        if(packet_time > it->second){
                            // cout << "ok2" << endl;
                            child.insert(std::pair<unsigned int,bool>(p3->getHeader()->getPreID(),0));
                        }
                        flag = 1;
                    }
                }
                if(!flag){
                    // cout << "ok3 " << packet_ID<<packet_time << endl;
                    child.insert(std::pair<unsigned int,bool>(p3->getHeader()->getPreID(),0));
                    packet_sequence.insert(pair<unsigned int,unsigned int>(packet_ID,packet_time));
                }
            }
            else if(l3->getParent_recieve(1) == getNodeID()){//刪除小孩
                // cout << "parent " << getNodeID() << " delete child " << p3->getHeader()->getPreID() << " packet time: " << packet_time << endl;
                map<unsigned int,unsigned int>::iterator it;
                int flag = 0;
                // cout << "~ok0" << endl;
                for(it = packet_sequence.begin();it!=packet_sequence.end();it++){
                    if(it->first == packet_ID){
                        ...
                        if(packet_time > it->second){
                            // cout << "~ok2" << endl;
                            child.erase(p3->getHeader()->getPreID());
                        }
                        flag = 1;
                    }
                }
        ...
        unsigned int New_ctrl_packet_ID = p3->getHeader()->getPreID();
        if(routing_table.at(New_ctrl_packet_ID)){
            return;
        }
        view(New_ctrl_packet_ID);
        if(hi){
            // cout << "check:" << getNodeID() << endl;
            if(level >= l3->getCounter()){
                if(New_ctrl_packet_ID < parent){//ID小的優先

                    unsigned delete_parent = parent;
                    parent = New_ctrl_packet_ID;
                    // cout << "old parent:" << delete_parent << " new parent:" << parent << endl;
                    level = l3->getCounter();
                    //insert parent+delete parent
                    IoT_ctrl_packet *ptp = nullptr;
                    ptp = dynamic_cast<IoT_ctrl_packet*>(p);
                    IoT_ctrl_payload *ptpy = nullptr;
                    ptpy = dynamic_cast<IoT_ctrl_payload*> (ptp->getPayload());
                    ptpy->setParent_check(1);
                    ptpy->setParent_recieve(parent,0);
                    ptpy->setParent_recieve(delete_parent,1);
                    ptpy->setID(sequence);
                    sequence++;
                    ...
                    // ptpd = dynamic_cast<IoT_ctrl_packet*>(p);
                    // IoT_ctrl_payload *ptpyd = nullptr;
                    // ptpyd = dynamic_cast<IoT_ctrl_payload*> (ptp->getPayload());
                    // ptpyd->setParent_check(1);
                    // ptpyd->setParent_recieve(delete_parent);
                    // ptpd->getHeader()->setPreID ( getNodeID() );
                    // ptpd->getHeader()->setNexID ( BROADCAST_ID );
                    // ptpd->getHeader()->setDstID ( BROADCAST_ID );
                    // send_handler(ptpd);
                    // cout << getNodeID() <<" "<< parent << "------------------" << endl;
                }
            }
        }
        else{
            ...
            hi = true;
            send_handler(ptp);
            // cout << "node:" <<getNodeID() <<"'s parent is"<< parent << endl;
        }
        ...
        IoT_data_packet *pd = nullptr;
        pd = dynamic_cast<IoT_data_packet*>(p);
        unsigned int check = pd->getHeader()->getPreID();
        // cout << "node " << getNodeID() << " get the packet from " << check << endl;
        map<unsigned int,bool>::iterator it;
        pd->getHeader()->setPreID(getNodeID());
        pd->getHeader()->setNexID(parent);
        pd->getHeader()->setDstID(0);
        // for(it = routing_table.begin();it!=routing_table.end();it++){
        //     cout << it->first << " " << it->second << "--------" << endl;
        // }
        // cout << child.begin()->first << endl;
        if(child.empty() && !send_check){
            send_check = true;
            send_handler(pd);
        }
        else{
        ...
        int source = p3->getHeader()->getSrcID();
        int packet_come = p3->getHeader()->getPreID();

        if(source != getNodeID()){
            rev_hop.insert(std::pair<unsigned int,unsigned int>(source,packet_come));
        }
        else{
        ...
        if(p3->getHeader()->getDstID() != getNodeID()){
            unsigned int next_hop = rev_hop[p3->getHeader()->getDstID()];
            p3->getHeader()->setPreID(getNodeID());
            p3->getHeader()->setNexID(next_hop); 
            send_handler(p3);
        }
        else{
            parent = l3->getParent();
...
void IoT_sink::recv_handler (packet *p){
    ...
    // node 0 broadcasts its message to every node and every node relays the packet "only once" and increases its counter
    // the variable hi is used to examine whether the packet has been received by this node before
    // you can remove the variable hi and create your own routing table in class IoT_sink
    if (p == nullptr) return ;
    ...
    if (p->type() == "IoT_ctrl_packet" && !hi ) { // the device receives a packet from the sink
        ...
        AGG_ctrl_payload *l3 = nullptr;
        l3 = dynamic_cast<AGG_ctrl_payload*> (p3->getPayload());
        int parent = stoi(l3->getMsg());
        // cout << p3->getHeader()->getSrcID() << parent << endl;
        device_parent.insert(pair<unsigned int,unsigned int>(p3->getHeader()->getSrcID(),parent));
        next_hop.insert(pair<unsigned int,unsigned int>(p3->getHeader()->getSrcID(),p3->getHeader()->getPreID()));
        // cout << p3->getHeader()->getSrcID() << " " << p3->getHeader()->getPreID() << endl;
        // cout << "node id = " << getNodeID() << ", msg = "  << l3->getMsg() << endl;
    }
    else if (p->type() == "DIS_ctrl_packet") {
        DIS_ctrl_packet *p3 = nullptr;
        ...
        l3 = dynamic_cast<DIS_ctrl_payload*> (p3->getPayload());
        p3->getHeader()->setPreID ( getNodeID() );
        for(auto it : device_parent){
            p3->getHeader()->setNexID ( next_hop[it.first] );
            p3->getHeader()->setDstID(it.first);
            l3->setParent(it.second);
            send_handler(p3);
        }
        // cout << "node id = " << getNodeID() << ", parent = "  << l3->getParent() << endl;
    }
...
int main()
{
    int Nodes,Links,SimTime,BFS_Start_Time,Data_Trans_Time,AGG_Start_Time,DIS_Start_Time;
    int packet_size;
    ...
    for(int i = 0;i < Links;i++){
            int temp,link1,link2;
            cin >> temp >> link1 >> link2;
            node::id_to_node(link1)->add_phy_neighbor(link2);
            node::id_to_node(link2)->add_phy_neighbor(link1);
            if(link1 != 0 && link2 != 0){
                device[link1]->table(link2);
                device[link2]->table(link1);
            }
    ...
    for(int id = 1;id < Nodes;id++){
        cout << id << " " << device[id]->getParent()<<endl;
    }
    ...
    // }
    // IoT_ctrl_packet_event(0, BFS_Start_Time);
    // event::start_simulate(Data_Trans_Time);
    // for(int id = 1;id < Nodes;id++){
    //     IoT_data_packet_event(id, 0, 100);
    // }
